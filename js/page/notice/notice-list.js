mui.init({
		...
		function loadNews(){
			...
			app.ajax("lf_notice/list",obj, function(data) {
				...
				for(var i = 0;i < datas.length; i++) {
					...
					li.id = news.id;
					var btnHmtl = 	'<div class="mui-table notice-list">'
			                        +	'<div class="mui-table-cell mui-col-xs-10">'
				                    +       '<h4 class="mui-ellipsis">'+news.title+'</h4>'
				                    +        '<p style="text-align:right" class="mui-h6 mui-ellipsis">'+news.add_time+'</p>'
				                    +    '</div>'
				                    +'</div>';
		...
		mui(".mui-table-view").on('tap','.mui-table-view-cell',function(){
		  	...
		  	mui.openWindow({url:'article.html',extras: {nid: nid}});
		});
